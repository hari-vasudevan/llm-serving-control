...
def wait_for_health(url: str, timeout_s: float = 300.0) -> None:
    deadline = time.time() + timeout_s
    last_error = ""
    while time.time() < deadline:
        try:
            status, text = get_text(f"{url.rstrip('/')}/health", timeout_s=5)
            if 200 <= status < 300:
            ...
            last_error = repr(exc)
        print(f"[health] waiting: {last_error}", flush=True)
        time.sleep(5)
